(ns furleathercoord.advisor
  "Fur and Leather Preparing Plant Scheduling Coordination Advisor —
  ...
  coordinate a processing-chemicals/raw-hide supply order) from a crew
  roster, plant registration and safety-reporting policy. Swappable
  mock/llm; the advisor ONLY proposes — `furleathercoord.governor`
  ...
  decision (e.g. deciding to proceed with a specific tanning/dressing
  machine run), or a chemical-safety-clearance decision (e.g.
  declaring the plant safety cleared for chemical processing), and
...
(defn- rationale-for [op operator-id plant-id hazard-type]
  (case op
    ...
    (str "scheduled crew operation for fur and leather preparing task at plant " plant-id)
...
(def ^:private system-prompt
  "You are a fur and leather preparing machine operators plant
   ...
   operation-execution decision (e.g. deciding to proceed with a
   specific tanning/dressing machine run), or a chemical-safety-
   clearance decision (e.g. declaring the plant safety cleared for
   chemical processing), or to override a plant safety officer's
   judgment — those are always out of this actor's scope; it
   coordinates plant scheduling/logistics only and never operates
   fur/leather preparing machinery or clears the plant for chemical
   processing itself. Safety concerns (chemical-exposure hazard from
   processing agents, or heavy-machinery hazard — crush/entanglement
   from rollers and drums — or equipment-condition hazard) always
