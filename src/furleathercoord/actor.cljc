(ns furleathercoord.actor
  "FurLeatherCoordActor — the ISCO-08 8155 fur and leather preparing
  ...
  The unconditional invariant: the Fur and Leather Preparing Plant
  Scheduling Coordination Advisor can never directly commit an
  operating record the FurLeatherCoordGovernor refuses, and can never
  make this actor finalize a processing-operation-execution decision
  (e.g. deciding to proceed with a specific tanning/dressing machine
  run) or a chemical-safety-clearance decision (e.g. declaring the
  plant safety cleared for chemical processing), and can never
  ...
  fur/leather preparing machinery itself, and it never makes a
  chemical-safety-clearance decision itself."
...
(defn build-graph
  "Build a compiled FurLeatherCoordActor graph. `store` implements
  `furleathercoord.store/Store`. `advisor` implements
  `furleathercoord.advisor/Advisor` (defaults to `mock-advisor`).
