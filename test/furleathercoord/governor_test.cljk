(ns furleathercoord.governor-test
  (:require [clojure.test :refer [deftest is testing]]
            [furleathercoord.store :as store]
            [furleathercoord.advisor :as advisor]
            [furleathercoord.governor :as governor]))

(defn- fresh-store []
  (let [st (store/mem-store)]
    (store/register-operator! st {:operator-id "operator-1" :name "Aki Sato"})
    (store/register-plant! st {:plant-id "P-1" :name "Kobo Fur and Leather Plant" :max-supply-cost 2000})
    st))
...
(deftest hard-on-scope-excluded-op-finalize-processing-operation
  (testing "finalizing the processing operation is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :finalize-processing-operation) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))
...
(deftest hard-on-scope-excluded-op-declare-plant-safety-cleared
  (testing "declaring the plant safety cleared for chemical processing is a permanent block, never a routine op"
    (let [st (fresh-store)
...
(deftest hard-on-scope-excluded-op-declare-batch-safe-for-handling
  (testing "declaring a dressed batch safe for handling is a permanent block, never a routine op"
    (let [st (fresh-store)
...
(deftest always-escalates-safety-concern-even-at-high-confidence
  (testing "a chemical-exposure/heavy-machinery(crush/entanglement from rollers and drums)/equipment-condition concern always requires human sign-off"
    (let [st (fresh-store)
...
(deftest always-escalates-safety-concern-machinery-hazard-even-at-high-confidence
  (testing "a heavy-machinery hazard (crush/entanglement from rollers and drums) concern always requires human sign-off"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :flag-safety-concern :hazard-type :machinery-entanglement)
                                           :confidence 0.99)
                             st)]
      (is (not (:hard? v)))
      (is (:escalate? v)))))
...
(deftest default-mock-advisor-proposals-never-self-trip-on-scope-exclusion
  (testing "the governor's scope-exclusion term list must never match the mock advisor's own default rationale text for any allowlisted op — CLAUDE.md's known self-tripping bug pattern (rationale legitimately contains bare nouns like 'pelt'/'fur'/'leather'/'chemical'/'tanning', but never the full finalization-action phrases)"
    (let [st (fresh-store)
      ...
      (doseq [o ops]
        (let [request {:operator-id "operator-1" :op o :plant-id "P-1"
                        :stake :low :task "routine fur and leather preparing task with tanning drum rollers and chemical processing agents"
